...
@st.dialog("Formulário de Cadastro de Setor Responsável - Manutenção", width="large")
def cad_novo_setor_responsavel_adm():
    ...
    with st.form(key="new_cad_form_setor_responsavel"):
        ...
        submit_button = st.form_submit_button(label="Cadastrar Setor Responsável")

    if submit_button:
        new_data_setor_responsavel = {
            "GID":str(cd_GID),
        ...
        try:
            with st.spinner("Salvando informações, por favor aguarde..."):
                create_data('timecenter.TB_CADASTRO_SETOR_RESPONSAVEL ', new_data_setor_responsavel)
                st.success("Novo registro cadastrado com sucesso!")
        except Exception as e:
            st.error(f"Erro ao realizar o novo registro na tabela timecenter.TB_CADASTRO_SETOR_RESPONSAVEL : {e}")
